...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-zinc-200/80 shadow-subtle px-2.5 sm:px-4 lg:px-6 py-2 sm:py-2.5">
      <div className="flex items-center justify-between gap-1.5 sm:gap-3">
        
        {/* Brand Title */}
        <div className="flex flex-col flex-shrink-0 select-none cursor-pointer">
          <span className="font-bold text-zinc-950 tracking-tight text-sm sm:text-base lg:text-lg font-sans">
            Kolkata Job Map
          </span>
          <span className="text-[10px] sm:text-[11px] font-medium text-zinc-400 -mt-0.5 hidden sm:block">
            Geospatial Tech Job Discovery
          </span>
        </div>

        {/* Global Search Bar */}
        <div className="flex-1 max-w-lg min-w-0 mx-1 sm:mx-2">
          <div className="relative flex items-center">
            <Search className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-zinc-400 absolute left-2.5 sm:left-3 pointer-events-none" />
            <input
              type="text"
              value={filters.searchQuery}
              onChange={(e) => updateFilter('searchQuery', e.target.value)}
              placeholder="Search companies, tech..."
              className="w-full pl-8 sm:pl-9 pr-7 sm:pr-8 py-1.5 sm:py-2 bg-zinc-50 hover:bg-zinc-100/80 focus:bg-white text-xs sm:text-sm text-zinc-900 rounded-xl border border-zinc-200/90 focus:border-zinc-400 focus:ring-2 focus:ring-zinc-900/5 focus:outline-none transition-all placeholder:text-zinc-400"
            />
            {filters.searchQuery && (
              <button
                onClick={() => updateFilter('searchQuery', '')}
                className="absolute right-2 p-0.5 text-zinc-400 hover:text-zinc-600 rounded-md"
              >
                <X className="w-3 h-3" />
              </button>
            )}
        ...
        {/* Starting Location Quick Button (Desktop XL) */}
        ...
        {/* Commute Mode Toggle (Desktop XL) */}
        <div className="hidden xl:flex items-center">
          <button
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0">
          ...
          >
            ...
            <span className="hidden md:inline">Fresher</span>
          </button>

          {/* Filter Drawer Toggle */}
          <button
            onClick={onToggleFilterDrawer}
            className={`relative flex items-center gap-1 px-2 sm:px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all ${
          ...
          <button
            ...
            className="relative flex items-center gap-1 px-2 sm:px-2.5 py-1.5 rounded-lg bg-zinc-900 text-white hover:bg-zinc-800 text-xs font-medium shadow-sm transition-all"
          ...
          <button
            ...
            className="hidden sm:flex p-1.5 sm:p-2 rounded-lg text-zinc-400 hover:text-zinc-800 hover:bg-zinc-100 transition-colors"
          ...
          <button
            ...
            className="md:hidden p-1.5 rounded-lg bg-zinc-100 text-zinc-800 border border-zinc-200"
